...
# Guardrail: if everything is 100%, the denominator is almost certainly wrong
if (nrow(region_summary) > 0) {
  vals <- region_summary$pct_intact_repaired
  vals <- vals[is.finite(vals)]
  if (length(vals) > 0 && length(unique(vals)) == 1 && unique(vals) == 100) {
    warning('Region % values are all 100%. This usually means the denominator grouping is wrong in region_summary.')
  }
}
...
for (b in sort(unique(locus_summary$batch))) {
  ...
  if (nrow(cis_top) > 0) {
    p_cis <- ggplot(cis_top, aes(x = allele, y = pct_within_group, fill = time_label)) +
      ...
      geom_text(aes(label = if_else(is.na(pct_within_group) | pct_within_group < 1, NA_character_, paste0(round(pct_within_group, 1), '%'))), position = position_dodge(width = 0.85), hjust = -0.05, size = 2.6, color = 'grey10') +
  ...
  if (nrow(reg_b) > 0) {
    p_region <- ggplot(reg_b, aes(x = region_class, y = pct_intact_repaired, fill = time_label)) +
      geom_col(position = position_dodge(width = 0.85), width = 0.8) +
      geom_text(aes(label = if_else(is.na(pct_intact_repaired) | pct_intact_repaired < 1, NA_character_, paste0(round(pct_intact_repaired, 1), '%'))), position = position_dodge(width = 0.85), vjust = -0.25, size = 2.5, color = 'grey10') +
