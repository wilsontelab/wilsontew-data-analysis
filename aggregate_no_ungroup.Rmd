...
# Experimental design metadata (3C pooled-strain experiment)
# NOTE:
# - DSB1 varies by strain and is sampled over the common locus panel.
# - DSB2 is batch-specific and MUST NOT be pooled across batches in one "DSB2" meta-group.
design_dsb2_by_batch <- tibble::tribble(
  ~batch, ~dsb2_chrom,
  "4", "VII",
  "6", "IV",
  "8", "XV"
...
# ---- Replicate-aware summary helper (mean/sd/sem across replicates) ----
summarize_across_replicates <- function(df_rep,
                                       group_cols_no_rep,
                                       metrics,
                                       replicate_col = COL_REPLICATE) {
  df_rep %>%
    group_by(across(all_of(group_cols_no_rep))) %>%
    summarise(
      n_reps = n_distinct(.data[[replicate_col]]),
      across(all_of(metrics), ~ mean(.x, na.rm = TRUE), .names = "{.col}_mean"),
      across(all_of(metrics), ~ sd(.x, na.rm = TRUE), .names = "{.col}_sd"),
      .groups = "drop"
    ) %>%
    mutate(
      across(
        ends_with("_sd"),
        ~ .x / sqrt(n_reps),
        .names = "{stringr::str_replace(.col, '_sd$', '_sem')}"
      )
    )
}
```
...
# - Default: ./ThreeC_Data (relative to the folder you opened)
# - Override: set WILSONTEW_3C_FOLDER
folder <- Sys.getenv("WILSONTEW_3C_FOLDER", unset = file.path(getwd(), "ThreeC_Data"))
...
message("Using folder: ", folder)

files <- list.files(folder, pattern = "_summary\\.csv$", full.names = TRUE)
if (length(files) == 0) {
  stop(
    "No *_summary.csv files found in folder: ", folder,
    "\n\nTo run locally, either copy your 3C summary CSVs into that folder,",
    " or set WILSONTEW_3C_FOLDER to the directory that contains them."
  )
}

raw_list <- lapply(files, function(f) readr::read_csv(f, show_col_types = FALSE))
dat_raw <- dplyr::bind_rows(raw_list)
...
dat <- dat_raw %>%
  ...
  mutate(
    ...
    # Replicate is a true replicate index (numeric)
    !!COL_REPLICATE := readr::parse_integer(as.character(.data[[COL_REPLICATE]])),
...
# ---- Design checks: pooled-strain / batch-specific DSB2 assumptions ----
timepoint_coverage <- dat %>%
  distinct(.data[[COL_BATCH]], .data[[COL_DSB]], .data[[COL_REPLICATE]], .data[[COL_TIME_POINT]]) %>%
  group_by(.data[[COL_BATCH]], .data[[COL_DSB]], .data[[COL_REPLICATE]]) %>%
  summarise(
    has_t0 = any(.data[[COL_TIME_POINT]] == 0),
    has_t120 = any(.data[[COL_TIME_POINT]] == 120),
    .groups = "drop"
  )

missing_timepoint_pairs <- timepoint_coverage %>%
  filter(!(has_t0 & has_t120))

if (nrow(missing_timepoint_pairs) > 0) {
  warning(
    "Some batch × DSB × replicate groups are missing T0 or T120. ",
    "Paired fold-change/correlation sections will automatically drop incomplete groups."
  )
}

design_batch_table <- dat %>%
  distinct(.data[[COL_BATCH]]) %>%
  mutate(batch = as.character(.data[[COL_BATCH]])) %>%
  left_join(design_dsb2_by_batch, by = "batch") %>%
  arrange(batch)

# ---- Guideline #3: spike-in scaling (batch × time_point) ----
spike_totals <- dat %>%
  filter(.data[[COL_ALLELE]] == SPIKEIN_ALLELE, .data[[COL_COMBO]] %in% combos_4) %>%
  group_by(across(all_of(c(COL_BATCH, COL_TIME_POINT, COL_REPLICATE)))) %>%
...
dat <- dat %>%
  left_join(
    spike_factors %>% select(all_of(c(COL_BATCH, COL_TIME_POINT, COL_REPLICATE)), SpikeIn_Scale, SpikeIn_Flag),
    by = c(COL_BATCH, COL_TIME_POINT, COL_REPLICATE)
  ) %>%
  mutate(
...
nocut_denoms <- dat %>%
  ...
  group_by(across(all_of(c(COL_BATCH, COL_TIME_POINT, COL_DSB, COL_COMBO, COL_REPEAT, COL_REPLICATE)))) %>%
  summarise(NoCut_Denom = sum(.data[[COL_COUNT_SPIKE_SCALED]], na.rm = TRUE), .groups = "drop")

dat <- dat %>%
  left_join(nocut_denoms, by = c(COL_BATCH, COL_TIME_POINT, COL_DSB, COL_COMBO, COL_REPEAT, COL_REPLICATE)) %>%
  mutate(
...
spike_factors %>%
  arrange(time_point, batch, replicate) %>%
  knitr::kable(caption = "Spike-in totals and scaling factors by batch × time_point × replicate")

nocut_denoms %>%
  arrange(time_point, batch, replicate, DSB, `repeat`, combo) %>%
  head(20) %>%
  knitr::kable(caption = "NO_CUT denominators (spike-scaled) by batch × time_point × replicate × DSB × repeat × combo (first 20)")

design_batch_table %>%
  knitr::kable(caption = "Batch-specific DSB2 chromosome map used for interpretation (do not pool DSB2 across batches)")

if (nrow(missing_timepoint_pairs) > 0) {
  missing_timepoint_pairs %>%
    knitr::kable(caption = "Groups with incomplete T0/T120 pairing (excluded from paired fold-change/correlation calculations)")
}
...
```{r aggregate-counts}
# (A) Pooled across replicates (old behavior)
dat_agg_counts_pooled <- summarize_combo4_counts_pct(
  dat_bio,
...
# (B) Replicate-level summary
dat_agg_counts_rep <- summarize_combo4_counts_pct(
  dat_bio,
  count_col = COL_COUNT_SPIKE_SCALED,
  group_cols = c(COL_BATCH, COL_TIME_POINT, COL_DSB, COL_REPLICATE),
  combo_col = COL_COMBO
)

# (C) Across-replicates summary (mean/sd/sem + n_reps)
metrics_counts <- c(
  "Total_Counts", "Cis_Counts", "Trans_Counts",
  "A_to_B_Counts", "C_to_D_Counts", "A_to_D_Counts", "C_to_B_Counts",
  "Percent_Cis", "Percent_Trans",
  "Percent_A_to_B", "Percent_C_to_D", "Percent_A_to_D", "Percent_C_to_B"
)

dat_agg_counts <- summarize_across_replicates(
  df_rep = dat_agg_counts_rep,
  group_cols_no_rep = c(COL_BATCH, COL_TIME_POINT, COL_DSB),
  metrics = metrics_counts
)

str(dat_agg_counts)
dat_agg_counts |> head(10) |> knitr::kable(
  caption = "Replicate-aware summary (mean/sd/sem across replicates) by batch × time_point × DSB (first 10)"
)
dat_agg_counts_pooled |> head(10) |> knitr::kable(
  caption = "Pooled summary (all replicates combined) by batch × time_point × DSB (first 10)"
)
```
...
group_cols_ab_rep <- c(COL_BATCH, COL_TIME_POINT, COL_DSB, COL_REPLICATE)

# Replicate-level summaries
cis_trans_ab_rep <- summarize_cis_trans_ab(
  dat_bio,
  count_col = COL_COUNT_NORM,
  group_cols = group_cols_ab_rep
)

intact_ssa_ab_rep <- summarize_intact_ssa_ab(
  dat_bio,
  count_col = COL_COUNT_NORM,
  group_cols = group_cols_ab_rep
)

# Replicate-level long form for plotting
cis_trans_long_rep <- cis_trans_ab_rep %>%
  select(any_of(group_cols_ab_rep), Percent_Cis, Percent_Trans) %>%
  pivot_longer(
    cols = c(Percent_Cis, Percent_Trans),
    names_to = "Class",
    values_to = "Percent"
...
intact_ssa_long_rep <- intact_ssa_ab_rep %>%
  select(any_of(group_cols_ab_rep), Percent_Intact, Percent_SSA) %>%
  pivot_longer(
...
# Fallback to spike-scaled counts if count_norm yields no usable values
if (nrow(cis_trans_long_rep) == 0 || !any(!is.na(cis_trans_long_rep$Percent))) {
...
if (nrow(intact_ssa_long_rep) == 0 || !any(!is.na(intact_ssa_long_rep$Percent))) {
...
# Across-replicates summaries (mean ± SEM)
cis_trans_ab_mean <- summarize_across_replicates(
  df_rep = cis_trans_ab_rep,
  group_cols_no_rep = group_cols_ab,
  metrics = c("Percent_Cis", "Percent_Trans"),
  replicate_col = COL_REPLICATE
)

intact_ssa_ab_mean <- summarize_across_replicates(
  df_rep = intact_ssa_ab_rep,
  group_cols_no_rep = group_cols_ab,
  metrics = c("Percent_Intact", "Percent_SSA"),
  replicate_col = COL_REPLICATE
)

# Mean long form for plotting
cis_trans_long_mean <- cis_trans_ab_mean %>%
  transmute(
    batch = .data[[COL_BATCH]],
    DSB = .data[[COL_DSB]],
    time_point = as.numeric(as.character(.data[[COL_TIME_POINT]])),
    n_reps,
    CIS_mean = Percent_Cis_mean,
    CIS_sem = Percent_Cis_sem,
    TRANS_mean = Percent_Trans_mean,
    TRANS_sem = Percent_Trans_sem
  ) %>%
  pivot_longer(
    cols = c(CIS_mean, CIS_sem, TRANS_mean, TRANS_sem),
    names_to = c("Class", ".value"),
    names_pattern = "(CIS|TRANS)_(mean|sem)"
  ) %>%
  rename(Percent_mean = mean, Percent_sem = sem)

intact_ssa_long_mean <- intact_ssa_ab_mean %>%
  transmute(
    batch = .data[[COL_BATCH]],
    DSB = .data[[COL_DSB]],
    time_point = as.numeric(as.character(.data[[COL_TIME_POINT]])),
    n_reps,
    INTACT_mean = Percent_Intact_mean,
    INTACT_sem = Percent_Intact_sem,
    SSA_mean = Percent_SSA_mean,
    SSA_sem = Percent_SSA_sem
  ) %>%
  pivot_longer(
    cols = c(INTACT_mean, INTACT_sem, SSA_mean, SSA_sem),
    names_to = c("Class", ".value"),
    names_pattern = "(INTACT|SSA)_(mean|sem)"
  ) %>%
  rename(Percent_mean = mean, Percent_sem = sem)
...
cis_trans_ab_mean %>%
  ...
  head(20) %>%
  knitr::kable(caption = "Preview: A/(A+B) normalized summaries across replicates (mean/sd/sem; first 20 rows)")

# Plot 1: CIS vs TRANS over time (replicates + mean ± SEM)
if (nrow(cis_trans_long_rep) > 0) {
  p_cis_trans_time <- ggplot() +
    geom_line(
      data = cis_trans_long_rep,
      aes(
        x = time_point,
        y = Percent,
        color = Class,
        group = interaction(Class, .data[[COL_REPLICATE]])
      ),
      alpha = 0.35,
      linewidth = 0.8
    ) +
    geom_point(
      data = cis_trans_long_rep,
      aes(x = time_point, y = Percent, color = Class),
      alpha = 0.5,
      size = 1.7
    ) +
    geom_errorbar(
      data = cis_trans_long_mean,
      aes(
        x = time_point,
        ymin = Percent_mean - Percent_sem,
        ymax = Percent_mean + Percent_sem,
        color = Class
      ),
      width = 6,
      linewidth = 0.7
    ...
    geom_line(
      data = cis_trans_long_mean,
      aes(x = time_point, y = Percent_mean, color = Class, group = Class),
      linewidth = 1.3
    ) +
    geom_point(
      data = cis_trans_long_mean,
      aes(x = time_point, y = Percent_mean, color = Class),
      size = 2.4
    ) +
    ...
    theme_bw() +
    labs(
      title = "CIS vs TRANS over time (replicates + mean ± SEM; A/(A+B) normalization)",
      x = "Time point",
      y = "% of (CIS + TRANS)",
      color = ""
    )
  ...
  message("[2.6 A/(A+B)] cis_trans_long_rep is empty; no CIS/TRANS A/(A+B) plot was produced.")
}

# Plot 2: INTACT vs SSA over time (replicates + mean ± SEM)
if (nrow(intact_ssa_long_rep) > 0) {
  p_intact_ssa_time <- ggplot() +
    geom_line(
      data = intact_ssa_long_rep,
      aes(
        x = time_point,
        y = Percent,
        color = Class,
    ...
    theme_bw() +
    labs(
      title = "INTACT vs SSA over time (replicates + mean ± SEM; A/(A+B) normalization)",
      x = "Time point",
      y = "% of (INTACT + SSA)",
      color = ""
    )
  ...
  message("[2.6 A/(A+B)] intact_ssa_long_rep is empty; no INTACT/SSA A/(A+B) plot was produced.")
}
```
...
# We intentionally do NOT produce a single pooled-across-batches DSB2 correlation panel.
# DSB2 is batch-specific in this experiment (batch 4 = ChrVII, 6 = ChrIV, 8 = ChrXV),
# so pooling DSB2 across batches creates a synthetic/non-biological "DSB2" group.
```
